...
for (time in time_span){
	...
	df_time = df_weekly_scaled[which(df_weekly$week==time),]  # CAMBIA DATASET
...
## loop fit
```{r}
var_sel <- all_cov_sample[1]
for(i in 1:n){
clusters_old = NULL
timing_result <- system.time({
for (time in time_span){
	cat(crayon::red("Time (ie week)",time,"\n"))
	df_time = df_weekly_scaled[which(df_weekly$week==time),]
	y_fit = df_time$AQ_pm10
	X = df_time[,var_sel] # tutte covariate tranne quelle inutili
...
if (i > 1 & LPML[i]>LPML[i-1])
{
	var_sel <- c(var_sel, var_sel[i])
}
}
