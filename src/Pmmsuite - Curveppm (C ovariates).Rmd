---
title: "title: \"Pmmsuite - Curveppm ( Covariates)\""
...
nsubject <- length(unique(df_weekly$IDStations)) # number of station
...
create Xcon and Xcat
```{r}
Xcon <- df_weekly %>% group_by(IDStations) %>% 
  summarise(Altitude = mean(Altitude), pigs = mean(LI_pigs)) %>% ungroup()
Xcon <- Xcon[,-1]
#Xcat <- df_weekly %>% group_by(IDStations) %>% 
  #summarise(WE_mode_wind_direction_100m = mean(WE_mode_wind_direction_100m))[,-1]
