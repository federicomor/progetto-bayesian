...
for (time in 1:53) {
	allXcons[[time]] = data.frame(
		Altitude = df_wsc$Altitude[seq(from=i, by = 53, length.out=105)],
		EM_nox_sum = df_wsc$EM_nox_sum[seq(from=i, by = 53, length.out=105)],
		WE_wind_speed_100m_max = df_wsc$WE_wind_speed_100m_max[seq(from=i, by = 53, length.out=105)],
		LA_lvi = df_wsc$LA_lvi[seq(from=i, by = 53, length.out=105)]
	)
	allXcats[[time]] = data.frame(
		WE_mode_wind_direction_100m = df_wsc$WE_mode_wind_direction_100m[seq(from=i, by = 53, length.out=105)]
	)
}
...
niter <- 20000
nburn <- 10000
nthin <- 20 # we consider every nthin iteartions 
