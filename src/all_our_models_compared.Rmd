...
## manual changes if we want to separately study some suspitious stations
# for example:
# cl_regions[[4]] = setdiff(cl_regions[[4]],80) # remove station 8 from group 4
# cl_regions[[5]] = 80 # add station 80 to group 5
# tssites$cl[80] = 5 # assign label 5 to station 5
...
```{r}
for(t in 1:20){
plot(FIT[[t]]$sig2[,sampled_station],type="l",
		 main=bquote("Trace plot of " * mu * " at time (=week) " * .(t) * 
...
```{r}
for(t in 1:18){
plot(FIT[[t]]$theta[sampled_station,t,],type="l",
		 main=bquote("Trace plot of " * theta * " at time (=week) " * .(t) * 
		 				" - station " * .(sampled_station)),
		 xlab = "MCMC iterations",ylab="values")
}
```
