...
for(time in time_span){
	...
	salso_out <- salso(t(drpm_model$Si[time,,]),
					   # loss=binder(),
					   loss=binder(a=1.2), # chosen one, better
					   maxNClusters = 6
					   )
...
## gif
```{r}
output_name = "binder 1p2"
percorso = paste0("./figures/Federico/final_drpm/",output_name,"/")
###################################
# save images
###################################
clusters_old = NULL
for(time in time_span){
	...
	p = plot_graph_and_hist(df_cluster_cut,cols)
	ggsave(file=paste0(percorso,output_name,"-",cur_num,".png"),plot=p,
