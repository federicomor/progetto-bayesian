...
for the wind
No, parte inutile, il wind non si riesce a plottarlo bene in nessun modo, passare oltre
```{r}
...
cols_gr = cols[1:length(unique(tssites$cl))]
plt_modemap = get_graph_plot(df_cluster_cut,titolo = "Mode clusters",cols=cols_gr,verbose=0)
print(plt_modemap)
...
# ggsave(paste0("./figures/Federico/time_series_tests/",cova_name,".png"),
# 	   plot = pts[[cova_name]],units="px",
...
# layout(matrix(c(1,1,2), nrow = 1, byrow = T))
...
plot(ys[1,],type="l",ylim=c(-2.4,1),col=cols[1],lwd=2,xlab="",ylab="")
...
Non sembra esserci differenza tra i cluster con i 2 diversi modelli. cambia leggermente il lpml.
