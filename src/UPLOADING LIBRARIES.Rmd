...
# Step 3 (?)
## on df_weekly
df_weekly già caricato da include.R
...
length(salso_out) # 105 valori, i cluster assegnati alle 105 obs
summary(salso_out)
```
...
df_temp = data.frame(
	...
	clusters = valori$cluster
)

# forse ci sta aggiungere un parametro a stationPlot, tipo (silent=TRUE)
# per scegliere se fargli stampare la mappa migliorata o se solo ritornarla
p = stationPlotgg <- stationPlot() +
	# geom_point(data =df_temp, aes(x=Longitude,y=Latitude),
	# 			   size=df_temp$values/10,
	# 			   color = df_temp$clusters, alpha=0.6)
	geom_point(data = df_temp, aes(x = Longitude, y = Latitude, color = factor(clusters)), size = 3)
print(p)
}
```
