...
Graph as before for each and all of the stations to see if any has interestingly distributed missed values
```{r}
...
for (st in stations[1:NSTAT]){
  # df_stat[[st]] = dati[which(dati$IDStations == st),]
  # assert(dim(df_stat[[st]])[1] == 2192) # make sure each df_stat has all his 2192 obs
  vis_miss(as.data.frame(df_stat[[st]]),warn_large_data = FALSE) #a plot for each station
}
```


```{r}
vis_miss(df_stat[["1264"]],warn_large_data = FALSE)
vis_miss(df_stat[["1265"]],warn_large_data = FALSE)
# ecc
```

#can see graphically for every variable
#notice how for some stations EM have na only on last year or so
#Don't think substituting with mean or median good idea in general, but we could do for EM using mean of first years of same station 
