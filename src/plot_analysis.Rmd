...
data_2016_lomb$Altitude <- as.numeric(data_2016_lomb$Altitude)
data_2016_lomb$Altitude[is.na(data_2016_lomb$Altitude)] <- 0
...
mappa_migliorata <- ggplot() +
  # l'ordine è importante!
	geom_sf(data = altre_regioni,aes(fill = station_inside),fill = "white", color = "lightgreen", size = 1) +
...
gradient_map <- 
  ...
  scale_fill_gradient(low = "lightgreen", high = "brown",na.value = "gray", breaks=c(0, 400, 800, 1200,1800,2000,2500,3000)) 
...
date_time = seq.Date(from=as.Date("2016-01-01",DATE_FORMAT),
				  to=as.Date("2020-12-01",DATE_FORMAT),by="month")
date_time = as.Date(date_time)

wind_arrows = wind_arrows[which(as.Date(df_chosen$Time) %in% date_time),]
...
```{r}
# debugging, one time frame
data_wind = wind_arrows[seq(1,1053*12*5,12*5),]
data_wind = wind_arrows

mappa_wind <- ggplot(data = data_wind) +
	geom_sf(data = shp_map, fill = "lightgreen", color = "black", size = 0.5)+
	coord_sf(xlim = range(na.omit(data_wind$longitude))+pad,
	  		 ylim = range(na.omit(data_wind$latitude))+pad, expand = FALSE)+

    geom_segment(data = data_wind,
