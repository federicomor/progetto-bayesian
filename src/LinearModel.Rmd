...
covs=covs[,-c(1,2,3)]   #lat, long and alti constant over same station
head(covs)
k=dim(covs)[2]
...
# forse dovremmo standardizzare le variabili
```
...
beta.post <- data.out[,1:30]
#posterior mean of the beta parameters
beta.bayes  <- apply(beta.post,2,"mean")
beta.bayes <- c(beta.bayes[30],beta.bayes[1:29])  #just to have beta0 in first position
beta.bayes
#sub-chain whit the sigma_res samle
sig.post= data.out[,'sigma']
```

Comparison between frequentist and bayesian linear models
```{r}
#### MLE estimate
mod= lm(y ~ covs[,1]+covs[,2]+covs[,3]+covs[,4]+covs[,5]+covs[,6]+covs[,7]+covs[,8]+covs[,9]+covs[,10]+covs[,11]+covs[,12]+
          ...
          covs[,25]+covs[,26]+covs[,27]+covs[,28]+covs[,29])
summary(mod)
...
# Cycle on all stations saving all betas 

datasets to store the betas
```{r}
...
#run model  
 ...
 #the betas
  beta.post <- data.out[,1:30]   #posterior mean of the beta parameters
  beta.bayes  <- apply(beta.post,2,"mean")
  beta.bayes <- c(beta.bayes[30],beta.bayes[1:29])  #just to put beta0 in first position
