...
# settings
```{r}
cols = colora(2,800,show=T)
color_low =  cols[2]
color_high = cols[1]

filter_date_list = filter_date(data_agc_lomb,"2016-01-01","2016-01-01","")
data_from_to = filter_date_list[[1]]

chosen_variable = as.numeric((data_from_to[,"Altitude"]))
...
p  <-  ggplot() +
	...
	scale_fill_gradient(low = color_low, high = color_high,na.value = "gray",trans = "sqrt",  # Apply a logarithmic transformation (adjust as needed)
...
qf = q +
	geom_sf(data = altre_regioni, fill = color_empty ,color = color_fill, linewidth = 0.1,alpha=0.01, show.legend = FALSE) +
	geom_sf(data = lombardia_2, fill = color_empty, color = "yellow", linewidth = 0.3,alpha=0.07, show.legend = FALSE) +
