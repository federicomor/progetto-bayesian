...
* alpha_0 = integer - logical indicating wether to update alpha or not.
...
void drpm_ar1_sppm2(int *draws, int *burn, int *thin, int *nsubject, int *ntime, 
			  double *y, double *s1, double *s2, double *M, 
			  double *modelPriors, int *global_alpha,  
			  int *alpha_0, int *eta1_0, int *phi1_0, 
			  ...
			  double *llike, double *lpml, double *waic, int *predSi){
...
//	RprintIVecAsMat("gamma_iter", gamma_iter, *nsubject, ntime1);
	...
	double summu, nt, ot, lam2tmp, phi1sq, sumt, op1, np1, ssq;
	...
	// priors for alpha
	double a = modelPriors[4], b = modelPriors[5];
	double al = modelPriors[6], bl = modelPriors[7];
//	Rprintf("A = %f\n", A);
//	Rprintf("m0 = %f\n", m0);
//	Rprintf("s20 = %f\n", s20);
//	Rprintf("alpha a = %f\n", a);
...
//					Rprintf("gamma_iter[j*(ntime1) + t] = %d\n", gamma_iter[j*(ntime1) + t]);
					if(gamma_iter[j*(ntime1) + t] == 0){
						...
						for(jj = 0; jj < *nsubject; jj++){
							...
							// I need to include this because need to determine what happens when 
...
//								Rprintf("lpp = %f\n", lpp);
							...
							if(t > 0){
//								Rprintf("t = %d\n", t);
//								Rprintf("Logdnorm = %f\n", dnorm(y[j*(*ntime) + t],muh[k*(ntime1) + t] + eta1_iter[j]*y[j*(*ntime) + t-1],sqrt(sig2h[k*(ntime1) + t]*(1-eta1_iter[j]*eta1_iter[j])), 1));
//								Rprintf("lpp = %f\n", lpp);
...
//							Rprintf("t = %d\n", t);
//							Rprintf("dnorm(y[j*(*ntime) + t],mudraw + eta1_iter[j]*y[j*(*ntime) + t-1],sigdraw*sqrt(1-eta1_iter[j]*eta1_iter[j]), 1) = %f\n", dnorm(y[j*(*ntime) + t], mudraw + eta1_iter[j]*y[j*(*ntime) + t-1],sigdraw*sqrt(1-eta1_iter[j]*eta1_iter[j]), 1));
//							Rprintf("lpp = %f\n", lpp);
